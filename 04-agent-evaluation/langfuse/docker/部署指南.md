...
# 4. 加密盐值（必改！）
SALT=your_random_salt_at_least_32_chars

# 5. Redis 密码（必改！）
REDIS_AUTH=your_strong_redis_password_here

# 6. MinIO 密码（必改！）
MINIO_ROOT_PASSWORD=your_strong_minio_password_min_8_chars
...
LANGFUSE_INIT_PROJECT_NAME=My First Project
```
...
3. 记录以下信息（用于 SDK 集成）:
   - `Public Key` (公钥)
   - `Secret Key` (私钥)
   - `Host` (服务器地址, 默认 `http://localhost:3000`)
...
| 变量名 | 说明 | 默认值 | 必改 |
|--------|------|--------|------|
| `NEXTAUTH_URL` | 应用访问 URL | `http://localhost:3000` | 生产环境必改 |
