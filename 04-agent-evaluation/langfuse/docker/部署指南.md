...
1. **首次访问**: 打开浏览器访问 `http://localhost:3000`（或您的服务器地址）
2. **注册账户**: 
   - 系统会自动跳转到注册页面
   - 填写以下信息：
...
3. **创建组织**:
   ...
   - 填写组织信息：
     ...
     - **Description**: 组织描述（可选）
   - 点击 **"Create"** 完成创建
4. **切换组织**: 如果创建了多个组织，可以在右上角切换

**注意**: 首次注册的用户会自动创建一个默认组织，通常无需手动创建。
...
3. **复制密钥**:
   - 点击 **"Show"** 按钮显示 Secret Key（首次显示后请妥善保存）
   - 点击 **"Copy"** 按钮复制密钥到剪贴板
